...
void Executor::processTimers(ExecutionState *current) {
  ...
  // TODO(bucur): This is broken, refactor
  if (ticks || dumpPTree || dumpStates) {
    ...
    if (instrTime > 0 && current && !removedStates.count(current)) {
      if (timerTicks*kSecondsPerTick > instrTime ) {
        LOG(WARNING) << "max-instruction-time exceeded: " << timerTicks*kSecondsPerTick;
